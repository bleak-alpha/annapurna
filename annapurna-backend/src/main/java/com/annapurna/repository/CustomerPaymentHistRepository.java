...
public interface CustomerPaymentHistRepository extends JpaRepository<CustomerPaymentHist, Integer> {
    ...
    @Query("SELECT cph FROM CustomerPaymentHist cph WHERE cph.creationDate BETWEEN :startDate AND :endDate ORDER BY cph.creationDate DESC")
    List<CustomerPaymentHist> findPaymentsBetweenDates(@Param("startDate") LocalDateTime startDate, 
